...
def load_ocr_result_for_image(output_dir: str, image_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not os.path.exists(json_path):
            print(f"Warning: OCR result not found: {json_path}", file=sys.stderr)
            return None
...
def main():
    parser = argparse.ArgumentParser(description='Calculate OCR accuracy')
    parser.add_argument('--ground_truth', default='images/labels.json', help='Path to the master ground truth JSON file')
    parser.add_argument('--output_dir', default='benchmark/results', help='Directory containing OCR output JSON files')
    parser.add_argument('--image_name', help='The specific image file name to process (e.g., image_0.png)')
    parser.add_argument('--batch', action='store_true', help='Batch mode: output JSON for all images')
    parser.add_argument('--debug', action='store_true', help='Enable debug mode')
    parser.add_argument('--results_file', help=argparse.SUPPRESS) # Suppress help for this unused arg

    args = parser.parse_args()

    # Batch mode: process all images and output JSON
    if args.batch:
        results_dict = {}
        ...
        # Process each image
        for image_name in gt_all.keys():
            gt_data = load_ground_truth_for_image(args.ground_truth, image_name)
            ocr_data = load_ocr_result_for_image(args.output_dir, image_name)
            
            if gt_data and ocr_data:
                metrics = calculate_research_standard_accuracy(gt_data, ocr_data, debug=False)
                results_dict[image_name] = {
                    'char_accuracy': metrics['character_accuracy'],
                    'cer': metrics['character_error_rate']
                }
        
        # Output JSON to stdout for run_benchmark.py to parse
        print(json.dumps(results_dict, ensure_ascii=False))
        return

    # Single image mode
    if not args.image_name:
        parser.error("--image_name is required when not using --batch")
        return
