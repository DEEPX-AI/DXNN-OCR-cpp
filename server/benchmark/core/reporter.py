...
class MarkdownReportGenerator(ReportGenerator):
    ...
    def generate(self, metrics: PerformanceMetrics, monitor: ResourceMonitor,
        ...
        if metrics.per_sample_stats:
            ...
            lines.append("| Sample Name | Runs | Success | Avg Latency (ms) | Chars | Pages | Accuracy (%) |")
            lines.append("|-------------|------|---------|------------------|-------|-------|--------------|")
            
            for sample_name, stats in sorted(metrics.per_sample_stats.items()):
                if 'error' not in stats:
                    acc_str = f"{stats['accuracy']:.2f}" if stats.get('accuracy') is not None else "-"
                    lines.append(
                        f"| `{sample_name}` | {stats['total_runs']} | {stats['successful_runs']} | "
                        f"{stats['avg_latency_ms']:.2f} | {stats['char_count']} | {stats['page_count']} | {acc_str} |"
                    )
            lines.append("")
...
class CSVReportGenerator(ReportGenerator):
    ...
    def generate(self, metrics: PerformanceMetrics, monitor: ResourceMonitor,
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            ...
            # 写入每个样本的结果
            writer.writerow(['Sample Name', 'Total Runs', 'Successful Runs', 'Avg Latency (ms)', 'Chars', 'Pages', 'Accuracy (%)'])
            for sample_name, stats in sorted(metrics.per_sample_stats.items()):
                if 'error' not in stats:
                    acc_val = f"{stats['accuracy']:.2f}" if stats.get('accuracy') is not None else ""
                    writer.writerow([
                        sample_name,
                        stats['total_runs'],
